"""
Prepare training data from Elasticsearch vitals data.
Uses REAL vitals data collected from the running system.
"""
import json
import os

MAX_TRAINING_SAMPLES = int(os.environ.get('MAX_TRAINING_SAMPLES', '1000'))

def generate_alerts(v):
    """Generate alerts based on vitals - EXACT match to original training logic."""
    alerts = []

    # Get values with flexible field names
    spo2 = v.get("spo2", v.get("oxygen_saturation", 100))
    hr = v.get("hr", v.get("heart_rate", 70))
    temp = v.get("temp", v.get("temperature", 37.0))
    bp_sys = v.get("bp_sys", v.get("bp_systolic", v.get("systolic", v.get("systolic_bp", 120))))
    bp_dia = v.get("bp_dia", v.get("bp_diastolic", v.get("diastolic", v.get("diastolic_bp", 80))))
    resp = v.get("resp", v.get("respiratory_rate", 16))
...
def prepare_training_data():
    """Prepare training data from collected Elasticsearch vitals."""
    training_pairs = []

    vitals_file = 'training_data/vitals.json'

    if not os.path.exists(vitals_file):
        print("No vitals data found at", vitals_file)
        return 0
    ...
    print(f"Found {len(hits)} vitals records from Elasticsearch")

    if len(hits) == 0:
        print("No vitals data in Elasticsearch")
        return 0
    ...
    for patient_id, vitals_list in patient_vitals.items():
        # Process in groups of 5 (matching original training)
        for i in range(0, len(vitals_list), 5):
            ...
            # Build input text (multiple vitals readings) - matching original format
            logs = []
            all_alerts = []
            for v in group:
                ...
                # Handle nested blood_pressure structure
                bp = v.get('blood_pressure', {})
                bp_sys = bp.get('systolic', v.get('systolic_bp', v.get('bp_systolic', 120)))
                bp_dia = bp.get('diastolic', v.get('diastolic_bp', v.get('bp_diastolic', 80)))
    ...
    disorders = sum(1 for d in training_pairs if "experienced" in d["summary"])
    stable = len(training_pairs) - disorders

    print(f"\nDataset statistics:")
    print(f"  - With disorders: {disorders} ({100*disorders/max(len(training_pairs),1):.1f}%)")
    print(f"  - Stable: {stable} ({100*stable/max(len(training_pairs),1):.1f}%)")
    ...
    print(f"\n✅ Prepared {len(training_pairs)} training examples (limit: {MAX_TRAINING_SAMPLES})")
